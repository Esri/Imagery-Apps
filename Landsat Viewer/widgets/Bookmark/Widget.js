...
function(declare, lang, array, html, BaseWidget, on, aspect, string,
  SpatialReference, ImageNode, TileLayoutContainer, utils, store, dom, registry,domStyle, Dialog, domConstruct, connect,PanelManager) {
...
ab :null,
    startup: function(){
      ...
      window.addEventListener("resize", lang.hitch(this, this.resizeBookmark));
        this.inherited(arguments);
       connect.publish("bookmarkopen", [{status : "open"}]);
...
resizeBookmark: function(){
//this.resize();
    if(window.innerWidth < 620){
      domStyle.set("Bookmark","font-size","7px");
      domStyle.set("Bookmark","width","200px");
      domStyle.set("Bookmark","height","200px");
      document.getElementById("Bookmark").childNodes[3].style.height="130px";
      document.getElementById("widgets/Bookmark/Widget_17").childNodes[5].style.height="109px";
  } else if(window.innerWidth < 850){
      domStyle.set("Bookmark","font-size","8px");
      domStyle.set("Bookmark","width","250px");
      domStyle.set("Bookmark","height","250px");
      document.getElementById("Bookmark").childNodes[3].style.height="200px";
     document.getElementById("widgets/Bookmark/Widget_17").childNodes[5].style.height="175px";
  } else if(window.innerWidth < 1200){
      domStyle.set("Bookmark","font-size","9px");
      domStyle.set("Bookmark","width","300px");
      domStyle.set("Bookmark","height","300px");
      document.getElementById("Bookmark").childNodes[3].style.height="250px";
      document.getElementById("widgets/Bookmark/Widget_17").childNodes[5].style.height="230px";
  } else {
      domStyle.set("Bookmark","font-size","12px");
      domStyle.set("Bookmark","width","350px");
      domStyle.set("Bookmark","height","350px");
      document.getElementById("Bookmark").childNodes[3].style.height="300px";
      document.getElementById("widgets/Bookmark/Widget_17").childNodes[5].style.height="280px";
  } 
  if(registry.byId("Bookmark").open){
      domStyle.set("Bookmark","top","70px");
      domStyle.set("Bookmark","left","160px");
  }
},
    onOpen: function(){
      // summary:
...
{
  connect.subscribe("buildopen", lang.hitch(this, function (buildopen) {
                        if (buildopen.buildstatus === "open")
                        {
                             buildopen.buildstatus = "close";
          console.log(pm);
    ...
    onClose: function(){
      ...
      this.bookmarks = [];
      this.currentIndex = -1;
  // connect.publish("bookmarkopen", [{status : "close"}]);  
    },
    ...
    _onBookmarkClick: function(bookmark) {
      ...
      registry.byId(this.label).onCancel();
      if(!this.myDialog) {
        ...
        this.myDialog.set("title", bookmark.name);
        this.myDialog.set("content", bookmark.description);
    }
            this.myDialog.show();
             
            domConstruct.destroy("bookmarkDialog_underlay");
    },

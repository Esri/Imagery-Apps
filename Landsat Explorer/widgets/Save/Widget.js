...
domClass, locale,popup,
          ...
          var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
            ...
            onOpen: function () {
                ...
                for(var a = this.map.layerIds.length - 1;a>=0;a--){
                   if(this.map.getLayer(this.map.layerIds[a]).url.indexOf("landsat") !== -1) {
                   var resultLayerProperties = this.map.getLayer(this.map.layerIds[a]);
                   break;}
               }
              /*  if(resultLayerProperties.renderingRule.rasterFunction)
                   var renderer = resultLayerProperties.renderingRule.rasterFunction;
               else
                   var renderer = resultLayerProperties.renderingRule.functionName;*/
               ...
               if(resultLayerProperties.id === "resultLayer"){
                ...
                registry.byId("itemTitle").set("value","Landsat layer - " +  locale.format(new Date(), {selector: "date", datePattern: "yyyy/MM/dd"})+ " - "+renderer);
                registry.byId("itemDescription").set("value","Landsat layer with " + renderer+" applied on it.");
           ...
           saveResultToArcGIS: function(){
               if(registry.byId("tooltipDialogIntro") && registry.byId("tooltipDialogIntro").state === "open" && registry.byId("tutorialStage").get("value") === "28"){
                   ...
                   this.showLoading();//domStyle.set("loadingSave","display","block");
                   for(var a = this.map.layerIds.length - 1;a>=0;a--){
                   if(this.map.getLayer(this.map.layerIds[a]).url.indexOf("landsat") !== -1) {
                   var resultLayerProperties = this.map.getLayer(this.map.layerIds[a]);
                   break;}
      ...
      var colormap = new RasterFunction();
              ...
              var renderingRule = resultLayerProperties.renderingRule.toJson();
       // var itemData = {"operationalLayers": [{"id": resultLayerProperties.id, "layerType": "ArcGISImageServiceLayer","url": resultLayerProperties.url, "visibility": true, "bandIds": bandIds,"opacity": 1, "title": registry.byId("itemTitle").get("value"), "timeAnimation": false, "renderingRule": renderingRule, "mosaicRule" : mosaicRule}], "baseMap": {"baseMapLayers": [{"id": "defaultBasemap_0", "layerType": "ArcGISTiledMapServiceLayer", "opacity": 1, "visibility": true, "url": "http://services.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer"}], "title": "Topographic"}, "spatialReference": {"wkid": 102100, "latestWkid": 3857}, "version": "2.1"};
         var itemData = {"id": resultLayerProperties.id, "visibility": true, "bandIds": bandIds,"opacity": 1, "title": registry.byId("itemTitle").get("value"), "timeAnimation": false, "renderingRule": renderingRule, "mosaicRule" : mosaicRule};
           ...
           var tempText = (bundle.identity.info).split("access the item on");
              ...
              addItemRequest.then(lang.hitch(this, function(result){
                 ...
                 this.hideLoading();//domStyle.set("loadingSave","display","none");
              }));  
              }));
            },

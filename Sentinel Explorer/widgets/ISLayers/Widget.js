...
define([
    ...
    'jimu/BaseWidget', "./resourceLoad.js", "dijit/TooltipDialog", "dojo/on"
],
    function (
        declare,
        _WidgetsInTemplateMixin, IdentityManager, MosaicRule,
        template,
        BaseWidget, resourceLoad, TooltipDialog, on
    ) {
        ...
        var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
            ...
            startup: function () {
                ...
                if (window.innerWidth < 620) {
                    ...
                    var headerCustom = domConstruct.toDom('<table id="headerTable" style="border: 0px;height: 40px;display: -webkit-inline-box;margin-left: 20px;">' +
                        '<tr style="height: 40px;"><td><div id="appName" style="font-size: ' + fontsize + 'px; position: relative; bottom: 3px; color: white; font-weight: bold;background-color: transparent;width:max-content;">Sentinel Explorer</div></td><td><div id="rendererInformation" style="font-size: ' + fontsize2 + 'px;color: white; font-weight: bold;background-color: transparent;display:none;width:max-content;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Rendering:&nbsp;Natural Color</div></td>' +
                ...
                new Tooltip({
                    connectId: ["appName"],
                    label: "Explore Sentinel Imagery with ArcGIS.",
                    position: ['below']
                });
            },
            ...
            postCreate: function () {
                ...
                // IdentityManager.registerToken({token: "fTJ56bd0vGgw19tB8EiTpJLfe7Kb5z-dVgehXwvKr-n7rzfGNEa18PZMWkjN6Wg-8wAEBwlriw5Nr4pnHChu7Cfjz16qXCzk2FF7lLCiKmdFVSfuJXv1xHbpen9Vnhhc", server: "https://www.arcgis.com/sharing/rest"});
                // this.map.useSecureService = true;
                // this.map.signOut = false;
                ...
                if (!this.storyModeOn) {
                    ...
                    var loadlayer = new ArcGISImageServiceLayer(this.config.sentinelUrl, {
                ...
                // on(document.getElementsByClassName('icon-node')[0], "click", lang.hitch(this, function() {
                ...
                //             //PanelManager.getInstance().closePanel(this.id);
                //         //}));
                //     }
                // })
            },
            onOpen: function () {
                this.map.signOut = true;
                var checkUrl = window.location.href;
                var parameters = window.location.href.split("?");
                if (parameters[1]) {
                    ...
                    if (values) {
                        ...
                        if (this.appRenderer === "Build") {
                            ...
                            registry.byId("bnd1").set("value", values[11]);
                            registry.byId("bnd2").set("value", values[12]);
                            registry.byId("bnd3").set("value", values[13]);
                            registry.byId("stretchoptions").set("value", values[14]);
                            registry.byId("gammaoptions").set("value", values[15]);

                        } else if (this.appRenderer === "Index") {

                            this.indexShareOpen = true;
                            this.appRenderer = values[16];
                            registry.byId("bandA").set("value", values[11]);
                            registry.byId("bandB").set("value", values[12]);
                            registry.byId("OffsetValue").set("value", values[13]);

                            registry.byId("Scale").set("value", values[14]);
                            registry.byId("colorRamp").set("value", values[15]);
                        }

                        this.secondaryLayerObject = values[4];
                        this.changeOrMask = values[10];
                    ...
                    if (typeof (history.pushState) !== "undefined") {
                        ...
                        var obj = { Page: "Sentinel Explorer", Url: changeUrl };
                    ...
                    if (e.credential.server.indexOf("arcgis.com") !== -1) {
                        if (document.getElementById("userName").innerHTML === "   Sign In") {
                            ...
                            this.portal.signIn().then(lang.hitch(this, function (user) {
                                ...
                                newSentinel.defaultBandIds = this.map.getLayer("primaryLayer").defaultBandIds;
                                newSentinel.defaultMosaicRule = this.map.getLayer("primaryLayer").defaultMosaicRule;
                                newSentinel.defaultRenderer = this.map.getLayer("primaryLayer").defaultRenderer;
                                newSentinel.defaultRenderingRule = this.map.getLayer("primaryLayer").defaultRenderingRule;
                                newSentinel.setRenderingRule(this.map.getLayer("primaryLayer").renderingRule);
                                newSentinel.extent = this.map.getLayer("primaryLayer").extent;
                                this.map.removeLayer(this.map.getLayer("primaryLayer"));
                                this.map.addLayer(newSentinel);
                                newSentinel.show();

                                this.config.sentinelUrl = "https://sentinel.arcgis.com/arcgis/rest/services/Sentinel2/ImageServer";
                            }));


                        } else {
                            if (e.credential.server === "https://sentinel.arcgis.com/arcgis") {
                                // if (document.getElementById("fullAccess")) {
                                //     domStyle.set("fullAccess", "display", "none");
                                // }
                                // if (registry.byId("timeDialog") && registry.byId("timeDialog").open) {
                                //     document.getElementById("fullAccessBtn").click();
                                // }
                                if (registry.byId('timeDialog')) {
                                    document.getElementsByClassName('icon-node')[0].click();
                                }
                                if (registry.byId("maskDialog") && registry.byId("maskDialog").open) {
                                    document.getElementsByClassName("icon-node")[2].click();
                                }
                                if (registry.byId("changeDetectionDialog") && registry.byId("changeDetectionDialog").open) {
                                    document.getElementsByClassName("icon-node")[3].click();
                                }
                            }
                        }
                    }
                ...
                IdentityManager.on("credentials-destroy", lang.hitch(this, function (e) {
                    ...
                    proxySentinel.defaultBandIds = this.map.getLayer("primaryLayer").defaultBandIds;
                    proxySentinel.defaultMosaicRule = this.map.getLayer("primaryLayer").defaultMosaicRule;
                    proxySentinel.defaultRenderer = this.map.getLayer("primaryLayer").defaultRenderer;
                    proxySentinel.defaultRenderingRule = this.map.getLayer("primaryLayer").defaultRenderingRule;
                    proxySentinel.setRenderingRule(this.map.getLayer("primaryLayer").renderingRule);
                    proxySentinel.extent = this.map.getLayer("primaryLayer").extent;
                    this.map.removeLayer(this.map.getLayer("primaryLayer"));
                    this.map.addLayer(proxySentinel);
                    proxySentinel.show();

                    this.config.sentinelUrl = "https://utility.arcgis.com/usrsvcs/servers/d70ebb358d28463a99e574d56265dd95/rest/services/Sentinel2/ImageServer";
                    if (registry.byId("timeDialog") && registry.byId("timeDialog").open) {
                        document.getElementsByClassName("icon-node")[0].click();
                    }
                    if (registry.byId("maskDialog") && registry.byId("maskDialog").open) {
            ...
            shareClicked: function (socialMedium) {
                if (!this.storyModeOn) {
                    ...
                    if (renderingRule !== '{"rasterFunction": "Build"}' && renderingRule !== '{"rasterFunction": "Index"}') {
                        var shareUrl = appUrl + "?" + btoa(mapextent + "&" + renderingRule + "&" + mosaicRule + "&" + cloudFilter + "&" + secondaryLayer + "&" + changeIndex + "&" + thresholdValue + "&" + changeMode + "&" + negativeRange + "&" + differenceRange + "&" + changeMaskFlag);
                    }
            ...
            userSignIn: function () {
                ...
                if (document.getElementById("userName").innerHTML === "   Sign In") {
                    if (domClass.contains(document.getElementsByClassName("icon-node")[0], "jimu-state-selected")) {
                        document.getElementsByClassName("icon-node")[0].click();
                    }
                    this.portal = new arcgisPortal.Portal("http://www.arcgis.com");
                    bundle.identity.lblItem = "Account";
            ...
            indexFunction: function (value) {
                ...
                if (layer.url === this.config.sentinelUrl && layer.format !== "lerc") {
                    ...
                    var newLayer = new ArcGISImageServiceLayer(this.config.sentinelUrl, {
                        imageServiceParameters: params,
                        visible: true,
                        id: "primaryLayer"
                    });
            ...
            reset1: function () {
                ...
                registry.byId("bnd1").set("value", "12");
                registry.byId("bnd2").set("value", "8");
                registry.byId("bnd3").set("value", "2");
                registry.byId("stretchoptions").set("value", "clip1");
                registry.byId("gammaoptions").set("value", "5");
                ...
                if (this.appRenderer) {
                    ...
                    if (this.appMosaicRule !== "null" && this.appMosaicRule) {
                        if (this.secondaryLayerObject !== "null")
                            registry.byId("appSceneID").set("value", this.appMosaicRule + "," + this.appCloudFilter + "," + true + "," + this.changeOrMask + "," + this.secondaryLayerObject);
                        else
                            registry.byId("appSceneID").set("value", this.appMosaicRule + "," + this.appCloudFilter + "," + false + "," + this.changeOrMask);
                        var x = document.getElementsByClassName("icon-node");
                        x[0].click();
                ...
                if (renderer === "None") {
                    ...
                    if (renderer !== "Build") {
                        ...
                        if (renderer !== "Index" && renderer !== "NDVI" && renderer !== "SAVI" && renderer !== "NBR" && renderer !== "NDWI" && renderer !== "NDMI" && renderer !== "Urban") {
                            ...
                            if (layer && layer.url === this.config.sentinelUrl && layer.format !== "lerc") {
                                layer.setBandIds([], true);
                            ...
                            if (!this.indexShareOpen) {
                                if (renderer === "NDVI") {
                                    registry.byId("bandA").set("value", 8);
                                    ...
                                    registry.byId("bandB").set("value", 12);
                                    ...
                                    registry.byId("bandA").set("value", 8);
                                    registry.byId("bandB").set("value", 13);
                                    registry.byId("colorRamp").set("value", "custom");
                                    dom.byId("formula").innerHTML = "Offset + Scale * ((NIR-SWIR_2)/(NIR + SWIR_2))";
                                } else if (renderer === "Urban") {
                                    registry.byId("bandA").set("value", 12);
                                    registry.byId("bandB").set("value", 8);
                                    registry.byId("colorRamp").set("value", "Precipitation");

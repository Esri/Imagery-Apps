            ...
            var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
                ...
                createHistogram: function (raster) {
                    ...
                    var request = new esriRequest({
                        url: this.config.urlSentinel + "/computehistograms",
                        content: {
                            f: "json",
                ...
                mosaicRuleApplied: function (layer) {
                    if (!this.stateClosed) {
                        if (this.newLayer && layer.target.url === this.config.urlSentinel) {
                ...
                onOpen: function () {
                    ...
                    if (!this.skipMaskFlag) {
                        ...
                        if (this.shareMaskRange)
                            ...
                            var mask = "NDVI";
                        if (this.map.getLayer("primaryLayer").url === this.config.urlSentinel || (this.map.getLayer("primaryLayer").mosaicRule && this.map.getLayer("primaryLayer").mosaicRule.method !== "esriMosaicLockRaster")) {
                            ...
                            query2.where = "Name = '" + registry.byId("primarySceneId").get("value") + "'";
                            query2.outFields = ["OBJECTID"];
                            query2.returnGeometry = false;

                            var queryTask2 = new QueryTask(this.config.urlSentinel);
                ...
                maskFunction: function (value) {
                    ...
                    if (value === "Urban") {
                        ...
                        var argNDVI = {};
                        argNDVI.BandIndexes = "(B8 - B4)/(B8 + B4)";
                        argNDVI.Method = 0;
                        ...
                        var argNDWI = {};
                        argNDWI.BandIndexes = "(B3 - B12)/(B3 + B12)";
                        argNDWI.Method = 0;
                    ...
                    if (this.map.getLayer("resultLayer") && registry.byId("changeMaskDetect").get("value") === "mask") {
                        ...
                        if (layer && layer.mosaicRule) {
                            ...
                            if (this.config.urlSentinel !== layer.url && layer.mosaicRule.method === "esriMosaicLockRaster")
                                params.mosaicRule.lockRasterIds = this.array;

                        }
                        params.format = "lerc";
                        params.lercVersion = 1;
                        this.newLayer = new RasterLayer(this.config.urlSentinel, {

...
define([
    ...
    'jimu/BaseWidget', "./resourceLoad.js", "dijit/TooltipDialog", "esri/layers/ImageServiceParameters", "esri/layers/RasterLayer", "dojo/on", "dojo/domReady!"
],
    function (
        declare,
        _WidgetsInTemplateMixin, IdentityManager, MosaicRule,
        template, ArcGISTiledMapServiceLayer,
        BaseWidget, resourceLoad, TooltipDialog, ImageServiceParameters, RasterLayer
    ) {
        ...
        var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
            ...
            startup: function () {
                ...
                if (window.innerWidth < 620) {
                    ...
                    var headerCustom = domConstruct.toDom('<table id="headerTable" style="border: 0px;height: 40px;display: -webkit-inline-box;margin-left: 20px;">' +
                        '<tr style="height: 40px;"><td><div id="appName" style="font-size: ' + fontsize + 'px; position: relative; bottom: 3px; color: white; font-weight: bold;background-color: transparent;width: max-content;">Ag.Explorer</div></td><td><div id="rendererInformation" style="font-size: ' + fontsize2 + 'px;color: white; font-weight: bold;background-color: transparent;display:none;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Rendering:&nbsp;Sentinel&nbsp;Natural Color</div></td>' +
                ...
                new Tooltip({
                    connectId: ["appName"],
                    label: "Explore Imagery with ArcGIS.",
                    position: ['below']
                });
                ...
                var naip = new ArcGISImageServiceLayer("https://naip.arcgis.com/arcgis/rest/services/NAIP/ImageServer", {
                    id: "naip123",
                    visible: false,
                    title: "USA NAIP Imagery",
                    imageServiceParameters: params
                });
                naip.hide();

                var dea = new ArcGISImageServiceLayer("https://sentinel.imagery1.arcgis.com/arcgis/rest/services/Sentinel2l2a/ImageServer", {
                    id: "dea123",
                    visible: false,
                    title: "SentinalL2A",
                    imageServiceParameters: params
                ...
                // var planet = new ArcGISImageServiceLayer("https://imageryais.geocloud.com/arcgis/rest/services/Planet/Planet_Conservation_CIR/ImageServer", {
                //     id: "planet123",
                //     visible: false,
                //     title: "Planet",
                //     imageServiceParameters: params
                // });
                // planet.setRenderingRule(new RasterFunction({ "rasterFunction": "None" }))
                // planet.hide(); 

                // var rasterParams = new ImageServiceParameters();
                // rasterParams.format = 'lerc';
                // var cropLayer = new ArcGISImageServiceLayer("https://landscape11.arcgis.com/arcgis/rest/services/USA_Cropland/ImageServer", {
                ...
                //   var rasterParams2 = new ImageServiceParameters();
                //   var colormap = new RasterFunction();
                //   colormap.functionName = 'Colormap';
                //   var argsc = {};
                //   argsc.Colormap = [[1,155,230,0],[2,108,203,255],[3,255,224,0],[4,134,134,134],[5,105,179,0],[6,56,153,204],[7,217,186,0],[-1,0,0,0],[16,0,0,0]];
                //   colormap.functionArguments = argsc;
                //   colormap.outputPixelType = 'U4';
                //   colormap.variableName = 'Raster';
                // rasterParams2.format = 'lerc';
                // rasterParams2.renderingRule = colormap;
                //   var hydroLayer = new RasterLayer("https://landscape11.arcgis.com/arcgis/rest/services/USA_Soils_Hydrologic_Group/ImageServer", {
                    
                //     imageServiceParameters: rasterParams2,
                //     visible: false,
                //     id: 'usa_hydro',
                //     title: 'USA Soils Hydrologic Group'
                //   });

                this.map.addLayers([landsat, sentinel, naip, dea]);
            ...
            onOpen: function () {
                ...
                var parameters = window.location.href.includes('?webmap=') ? window.location.href.split(window.location.href.substring(window.location.href.indexOf('?webmap='), window.location.href.indexOf('?webmap=') + 41)) : window.location.href.split("?");
                if (parameters[1]) {
                    ...
                    if (values) {
                        ...
                        if (this.appRenderer === "Build") {
                            registry.byId("redBuild").set("value", values[18]);
                            registry.byId("blueBuild").set("value", values[19]);
                            registry.byId("greenBuild").set("value", values[20]);
                            registry.byId("stretchBuild").set("value", values[21]);
                            registry.byId("gammaBuild").set("value", values[22]);
                        } else if (this.appRenderer === "Index") {
                            registry.byId("bandAIndex").set("value", values[18]);
                            registry.byId("bandBIndex").set("value", values[19]);
                            registry.byId("offsetIndex").set("value", values[20]);
                            registry.byId("scaleIndex").set("value", values[21]);
                            registry.byId("renderIndex").set("value", values[22]);
                        ...
                        if (values[2]) {
                            ...
                            if (this.secondaryAppRend === "Build") {
                                registry.byId("redBuildSec").set("value", values[23]);
                                registry.byId("blueBuildSec").set("value", values[24]);
                                registry.byId("greenBuildSec").set("value", values[25]);
                                registry.byId("stretchBuildSec").set("value", values[26]);
                                registry.byId("gammaBuildSec").set("value", values[27]);
                            } else if (this.secondaryAppRend === "Index") {
                                registry.byId("bandAIndexSec").set("value", values[23]);
                                registry.byId("bandBIndexSec").set("value", values[24]);
                                registry.byId("offsetIndexSec").set("value", values[25]);
                                registry.byId("scaleIndexSec").set("value", values[26]);
                                registry.byId("renderIndexSec").set("value", values[27]);
                        ...
                        if (values[6] !== "null") {
                         ...
                         //#270
                        this.exploreImagery = values[16];
                        this.compareImagery = values[17];

                        this.appCloudFilter = values[7];
                        //issue #265
                        if (this.appCloudFilter) {
                            registry.byId('cloudValue').set('value', this.appCloudFilter);
                        }
                        if (values[8]) {
                            this.appCloudFilterLeft = values[8];
                            registry.byId('cloudValueLeft').set('value', this.appCloudFilterLeft);
                        }
                        if (values[9]) {
                            this.appCloudFilterRight = values[9];
                            registry.byId('cloudValueRight').set('value', this.appCloudFilterRight);
                        }
                        


                        //this.secondaryLayerObject = values[5];
                        this.changeOrMask = values[15];
                        ...
                        if (this.changeOrMask === "mask") {
                            registry.byId("maskSliderReceiveValue").set("value", values[11]);
                            registry.byId("methodValue").set("value", values[10]);
                        } else if (this.changeOrMask === "change") {
                            registry.byId("methodValue").set("value", values[10]);
                            registry.byId("modeValue").set("value", values[12]);
                        }
                    ...
                    if (typeof (history.pushState) !== "undefined") {
                        ...
                        var obj = { Page: "Ag.Explorer", Url: changeUrl };
            ...
            shareClicked: function (socialMedium) {
                if (!this.storyModeOn) {
                    ...
                    if (registry.byId("cloudFilterView")) {
                        var cloudFilter = registry.byId("cloudFilterView").get("value");
                    }
                    else {
                    ...
                    if (registry.byId('exploreImageryOnOff').get('value')) {
                        var exploreImageryOnOff = registry.byId('exploreImageryOnOff').get('value');
                    } else {
                        var exploreImageryOnOff = 'off';
                    }

                    if (registry.byId('compareImageryOnOff').get('value')) {
                        var compareImageryOnOff = registry.byId('compareImageryOnOff').get('value');
                    } else {
                        var compareImageryOnOff = 'óff';
                    }
                    ...
                    if (renderingRule !== '{"rasterFunction": "Build"}' && renderingRule !== '{"rasterFunction": "Index"}' && renderingRuleSec !== '{"rasterFunction": "Build"}' && renderingRuleSec !== '{"rasterFunction": "Index"}') {
                        var shareUrl = appUrl + "?" + btoa(mapextent + "&" + renderingRule + "&" + renderingRuleSec + "&" + sensorName + "&" + secondarySensor + "&" + mosaicRule + "&" + secondaryMosaic + "&" + cloudFilter + "&" + cloudFilterLeft + "&" + cloudFilterRight + "&" + changeIndex + "&" + thresholdValue + "&" + changeMode + "&" + negativeRange + "&" + differenceRange + "&" + changeMaskFlag + "&" + exploreImageryOnOff + "&" + compareImageryOnOff);
                    }
                    else if (renderingRule === '{"rasterFunction": "Build"}' && renderingRuleSec === '{"rasterFunction": "Build"}') {
                        var shareUrl = appUrl + "?" + btoa(mapextent + "&" + renderingRule + "&" + renderingRuleSec + "&" + sensorName + "&" + secondarySensor + "&" + mosaicRule + "&" + secondaryMosaic + "&" + cloudFilter + "&" + cloudFilterLeft + "&" + cloudFilterRight + "&" + changeIndex + "&" + thresholdValue + "&" + changeMode + "&" + negativeRange + "&" + differenceRange + "&" + changeMaskFlag + "&" + exploreImageryOnOff + "&" + compareImageryOnOff + "&" + registry.byId("redBuild").get("value") + "&" + registry.byId("blueBuild").get("value") + "&" + registry.byId("greenBuild").get("value") + "&" + registry.byId("stretchBuild").get("value") + "&" + registry.byId("gammaBuild").get("value") + "&" + registry.byId("redBuildSec").get("value") + "&" + registry.byId("blueBuildSec").get("value") + "&" + registry.byId("greenBuildSec").get("value") + "&" + registry.byId("stretchBuildSec").get("value") + "&" + registry.byId("gammaBuildSec").get("value"));
                    }
                    else if (renderingRule === '{"rasterFunction": "Index"}' && renderingRuleSec === '{"rasterFunction": "Index"}') {
                        var shareUrl = appUrl + "?" + btoa(mapextent + "&" + renderingRule + "&" + renderingRuleSec + "&" + sensorName + "&" + secondarySensor + "&" + mosaicRule + "&" + secondaryMosaic + "&" + cloudFilter + "&" + cloudFilterLeft + "&" + cloudFilterRight + "&" + changeIndex + "&" + thresholdValue + "&" + changeMode + "&" + negativeRange + "&" + differenceRange + "&" + changeMaskFlag + "&" + exploreImageryOnOff + "&" + compareImageryOnOff + "&" + registry.byId("bandAIndex").get("value") + "&" + registry.byId("bandBIndex").get("value") + "&" + registry.byId("offsetIndex").get("value") + "&" + registry.byId("scaleIndex").get("value") + "&" + registry.byId("renderIndex").get("value") + "&" + registry.byId("bandAIndexSec").get("value") + "&" + registry.byId("bandBIndexSec").get("value") + "&" + registry.byId("offsetIndexSec").get("value") + "&" + registry.byId("scaleIndexSec").get("value") + "&" + registry.byId("renderIndexSec").get("value"));
                    }
                    else if (renderingRule === '{"rasterFunction": "Index"}' && renderingRuleSec === '{"rasterFunction": "Build"}') {
                        var shareUrl = appUrl + "?" + btoa(mapextent + "&" + renderingRule + "&" + renderingRuleSec + "&" + sensorName + "&" + secondarySensor + "&" + mosaicRule + "&" + secondaryMosaic + "&" + cloudFilter + "&" + cloudFilterLeft + "&" + cloudFilterRight + "&" + changeIndex + "&" + thresholdValue + "&" + changeMode + "&" + negativeRange + "&" + differenceRange + "&" + changeMaskFlag + "&" + exploreImageryOnOff + "&" + compareImageryOnOff + "&" + registry.byId("bandAIndex").get("value") + "&" + registry.byId("bandBIndex").get("value") + "&" + registry.byId("offsetIndex").get("value") + "&" + registry.byId("scaleIndex").get("value") + "&" + registry.byId("renderIndex").get("value") + "&" + registry.byId("redBuildSec").get("value") + "&" + registry.byId("blueBuildSec").get("value") + "&" + registry.byId("greenBuildSec").get("value") + "&" + registry.byId("stretchBuildSec").get("value") + "&" + registry.byId("gammaBuildSec").get("value"));
                    }
                        ...
                        var shareUrl = appUrl + "?" + btoa(mapextent + "&" + renderingRule + "&" + renderingRuleSec + "&" + sensorName + "&" + secondarySensor + "&" + mosaicRule + "&" + secondaryMosaic + "&" + cloudFilter + "&" + cloudFilterLeft + "&" + cloudFilterRight + "&" + changeIndex + "&" + thresholdValue + "&" + changeMode + "&" + negativeRange + "&" + differenceRange + "&" + changeMaskFlag + "&" + exploreImageryOnOff + "&" + compareImageryOnOff + "&" + registry.byId("bandAIndex").get("value") + "&" + registry.byId("bandBIndex").get("value") + "&" + registry.byId("offsetIndex").get("value") + "&" + registry.byId("scaleIndex").get("value") + "&" + registry.byId("renderIndex").get("value"));
                    }
                    else if (renderingRule !== '{"rasterFunction": "Build"}' && renderingRule !== '{"rasterFunction": "Index"}' && renderingRuleSec === '{"rasterFunction": "Build"}') {
                        var shareUrl = appUrl + "?" + btoa(mapextent + "&" + renderingRule + "&" + renderingRuleSec + "&" + sensorName + "&" + secondarySensor + "&" + mosaicRule + "&" + secondaryMosaic + "&" + cloudFilter + "&" + cloudFilterLeft + "&" + cloudFilterRight + "&" + changeIndex + "&" + thresholdValue + "&" + changeMode + "&" + negativeRange + "&" + differenceRange + "&" + changeMaskFlag + "&" + exploreImageryOnOff + "&" + compareImageryOnOff + "&" + null + "&" + null + "&" + null + "&" + null + "&" + null + "&" + registry.byId("redBuildSec").get("value") + "&" + registry.byId("blueBuildSec").get("value") + "&" + registry.byId("greenBuildSec").get("value") + "&" + registry.byId("stretchBuildSec").get("value") + "&" + registry.byId("gammaBuildSec").get("value"));
                    }
                    else if (renderingRule !== '{"rasterFunction": "Build"}' && renderingRule !== '{"rasterFunction": "Index"}' && renderingRuleSec === '{"rasterFunction": "Index"}') {
                        var shareUrl = appUrl + "?" + btoa(mapextent + "&" + renderingRule + "&" + renderingRuleSec + "&" + sensorName + "&" + secondarySensor + "&" + mosaicRule + "&" + secondaryMosaic + "&" + cloudFilter + "&" + cloudFilterLeft + "&" + cloudFilterRight + "&" + changeIndex + "&" + thresholdValue + "&" + changeMode + "&" + negativeRange + "&" + differenceRange + "&" + changeMaskFlag + "&" + exploreImageryOnOff + "&" + compareImageryOnOff + "&" + null + "&" + null + "&" + null + "&" + null + "&" + null + "&" + registry.byId("bandAIndexSec").get("value") + "&" + registry.byId("bandBIndexSec").get("value") + "&" + registry.byId("offsetIndexSec").get("value") + "&" + registry.byId("scaleIndexSec").get("value") + "&" + registry.byId("renderIndexSec").get("value"));
                    }
                } else {
                ...
                if (socialMedium === "facebook") {
                    var share = "http://www.arcgis.com/home/socialnetwork.html?n=fb&t=Ag.Explorer" + "&u=" + shareUrl;
                }
                else if (socialMedium === "twitter") {
                    var share = "http://www.arcgis.com/home/socialnetwork.html?n=tw&t=Ag.Explorer" + "&u=" + shareUrl;
                }
                else {

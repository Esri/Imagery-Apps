...
define([
    ...
    'jimu/BaseWidget', "./resourceLoad.js", "dojo/Evented", 'dijit/_TemplatedMixin', "dojo/_base/array", "dijit/form/NumberTextBox",
],
    function (
        declare,
        _WidgetsInTemplateMixin,
        template,
        BaseWidget, resourceLoad, Evented, _TemplatedMixin, arrayDojo, NumberTextBox
    ) {
        ...
        var clazz = declare([BaseWidget, Evented, _TemplatedMixin], {
            ...
            startup: function () {
                ...
                //registry.byId("temporal-year").on("change", lang.hitch(this, this.setDateRange));
            },
            ...
            onOpen: function () {
                ...
                this.autoresize();
                registry.byId('start-temporal').on("click", lang.hitch(this, function() {
                    this.addGraphic(this.map.aoi ? this.map.aoi : this.map.extent);
                }));
            ...
            refreshDataTempo: function () {
                ...
                } else if (this.primaryLayer.id === "Sentinel2_2553" || this.primaryLayer.id === "dea123") {
                    this.dateFieldTempo = "acquisitiondate";
                    this.categoryTempo = "category";
                    this.cloudTempo = "cloudcover";
                } else if (this.primaryLayer.id === "naip123") {
                    this.dateFieldTempo = "AcquisitionDate";
                    this.categoryTempo = "Category";
                } else if (this.primaryLayer.id === "planet123") {
                    this.dateFieldTempo = "AcqDate";
                    this.categoryTempo = "Category";
                } 
            },
            ...
            addGraphic: function (geometry) {
                ...
                if (geometry.type === 'extent') {
                    var point_temp = geometry.getCenter();
                } else if (geometry.type === 'polygon') {
                    var point_temp = geometry.getCentroid();
                }
                var graphic = new esri.Graphic(point_temp, symbol);
                this.map.graphics.add(graphic);
                this.pointGeometry = point_temp;
                this.temporalprof(point_temp);
            },

            setDateRange: function(val) {
                if (val === "2weeks") {
                    var currentDate = new Date(Date.now() - 14*24*60*60*1000);
                    this.temporalDateRange = this.dateFieldTempo + " >= date'" + currentDate.getFullYear() + "-" + (currentDate.getMonth() + 1) + "-" + currentDate.getDate() + " 12:00:00'";
                } else if (val === "1month") {
                    var currentDate = new Date(Date.now() - 31*24*60*60*1000);
                    this.temporalDateRange = this.dateFieldTempo + " >= date'" + currentDate.getFullYear() + "-" + (currentDate.getMonth() + 1) + "-" + currentDate.getDate() + " 12:00:00'";
                } else if (val === "3month") {
                    var currentDate = new Date(Date.now() - 92*24*60*60*1000);
                    this.temporalDateRange = this.dateFieldTempo + " >= date'" + currentDate.getFullYear() + "-" + (currentDate.getMonth() + 1) + "-" + currentDate.getDate() + " 12:00:00'";
                } else if (val === "1yearago") {
                    var dateA = new Date(Date.now() - 380*24*60*60*1000);
                    var dateB = new Date(Date.now() - 250*24*60*60*1000);
                    this.temporalDateRange = this.dateFieldTempo + " >= date'" + dateA.getFullYear() + "-" + (dateA.getMonth() + 1) + "-" + dateA.getDate() + " 12:00:00' AND " + this.dateFieldTempo + " <= date '" + dateB.getFullYear() + "-" + (dateB.getMonth() + 1) + "-" + dateB.getDate() + " 12:00:00'";
                } else if (val === "1yearfull") {
                    var currentDate = new Date(Date.now() - 400*24*60*60*1000);
                    this.temporalDateRange = this.dateFieldTempo + " >= date'" + currentDate.getFullYear() + "-" + (currentDate.getMonth() + 1) + "-" + currentDate.getDate() + " 12:00:00'";
                }
            },
            ...
            temporalprof: function (evt) {
                ...
                if (!this.noSceneFlag) {
                    ...
                    this.setDateRange(registry.byId("temporal-year").get("value"));
                    var point = evt;
                    var query = new Query();
                    ...
                    if (registry.byId('imageViewer').get('checked')) {
                        if (this.primaryLayer.url === this.config.urlSentinel || this.primaryLayer.url === this.config.urlDEA) {
                            query.where = this.categoryTempo + " = 1 AND " + this.cloudTempo + "<=" + registry.byId("cloudFilterView").get("value") + " AND " + this.temporalDateRange;


                        } else if (this.primaryLayer.url === this.config.urlNaip || this.primaryLayer.url === this.config.urlPlanet) {
                            query.where = this.categoryTempo + " = 1";
                        ...
                        if (this.primaryLayer.url === this.config.urlSentinel || this.primaryLayer.url === this.config.urlDEA) {
                            query.where = this.categoryTempo + " = 1 AND " + this.cloudTempo + "<=1 AND " + this.temporalDateRange;
                    ...
                    queryTask.execute(query, lang.hitch(this, function (result) {
                        ...
                        if (this.primaryLayer.url === this.config.urlSentinel) {
                            ...
                            if (distance.length > 20) {
                                var limitSamples = distance.length - 20;
                            } else {
                                var limitSamples = 0;
                            }
                            ...
                            if (distance.length !== 0) {
                                if (!registry.byId("imageViewer").checked) {
                                    ...
                                    if (distance.length > 20) {                                     //condition if samples are more than 10
                        ...
                        } else if (this.primaryLayer.url === this.config.urlPlanet) {
                            for (var i = 0; i < this.lengthofsamples; i++) {
                                if (i === 0) {
                                    distance.push({
                                        objectId: data[i].attributes.OBJECTID,
                                        acqDate: data[i].attributes.AcqDate,
                                        name: data[i].attributes.Name
                                    });
                                } else {
                                    if (locale.format(new Date(distance[distance.length - 1].acqDate), { selector: "date", datePattern: "dd/MM/yy" }) !== locale.format(new Date(data[i].attributes.AcqDate), { selector: "date", datePattern: "dd/MM/yy" })) {
                                        distance.push({
                                        objectId: data[i].attributes.OBJECTID,
                                        acqDate: data[i].attributes.AcqDate,
                                        name: data[i].attributes.Name
                            ...
                            if (distance.length !== 0) {
                                if (!registry.byId("imageViewer").checked) {
                                    ...
                                    var count = 0;
                                    for (var i = 0; i < distance.length; i++) {
                                        if (distance[i].acqDate === this.mainConfig.orderedFeatures[this.mainConfig.valueSelected].attributes.AcqDate) {
                                            count++;
                                        }
                                    }
                                    if (count === 0) {
                                        distance.push({
                                            objectId: this.mainConfig.orderedFeatures[this.mainConfig.valueSelected].attributes.OBJECTID,
                                            acqDate: this.mainConfig.orderedFeatures[this.mainConfig.valueSelected].attributes.AcqDate,
                                            name: this.mainConfig.orderedFeatures[this.mainConfig.valueSelected].attributes.Name
                                        });
                                    }
                                    ...
                                    arrayDojo.map(distance, lang.hitch(this, function (item, index) {
                                        if (item.acqDate === this.mainConfig.orderedFeatures[this.mainConfig.valueSelected].attributes.AcqDate) {
                                            this.objectIndex = index;
                                        }
                                    }));
                                    if (distance.length > 10) {                                     //condition if samples are more than 10
            ...
            moveImage: function (featureIds) {
                ...
                } else if (temporalLayer.id === "Sentinel2_2553" || temporalLayer.id === "dea123") {
                    this.acDate = "acquisitiondate";
                    this.titleSensor = "SENTINEL";
                } else if (temporalLayer.id === "naip123") {
                    this.acDate = "AcquisitionDate";
                    this.titleSensor = "NAIP";
                } else if (temporalLayer.id === "planet123") {
                    this.acDate = "AcqDate";
                    this.titleSensor = "PLANET";
                }
